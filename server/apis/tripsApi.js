    ...
    async.parallel({
        ...
        if(populateErr){
            ...
            for(var i=0;i<vehicleIds.length;i++) {
                ...
                //console.log(vehicle);
                var vehicleInfo = _.find(populateResults.tripFreightTotal, function (total) {
                    if(total._id === vehicle.registrationNo) {
                        return total;
                    }
                });
                if(vehicleInfo){
                ...
                vehicleInfo =  _.find(populateResults.expensesTotal, function (expense) {
                    if(expense.vehicleNumber === vehicle._id){
                        return expense;
                    }
                });
                if(vehicleInfo){
            ...
            Utils.populateNameInTrucksColl(vehicles,'registrationNo', function(result){
                retObj.status = true;
                retObj.messages.push('Success');
                retObj.vehicles = result.documents;
                callback(retObj);
            })
        }
    });
