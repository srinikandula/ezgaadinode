...
app.controller('AddEditInvoiceCtrl', [
  '$scope',
  'PartyService',
  'Notification',
  'InvoiceService',
  ...
  function (
    ...
    PartyService.getAllPartiesForFilter(
    ...
    $scope.getTrips = function () {
      TripServices.getAllAccountTrips(
        [],
        function (successCallback) {
    ...
    TrucksService.getAllTrucksForFilter(
      function (successCallback) {
          ...
          if (successCallback.data.status) {
            ...
            if ($scope.invoice.addTrip) {
              ...
              for (var i = 0; i < $scope.invoice.trip.length; i++) {
                $scope.temp = true;
                $scope.invoice.trip[i].date = new Date(
                  $scope.invoice.trip[i].date
                );
              }
            }
            for (var i = 0; i < $scope.invoice.trip.length; i++) {
              ...
              if ($scope.invoice.trip[i].loadedOn !== undefined) {
                $scope.invoice.trip[i].loadedOn = new Date(
                  $scope.invoice.trip[i].loadedOn
                );
              }
              if ($scope.invoice.trip[i].unloadedOn !== undefined) {
                $scope.invoice.trip[i].unloadedOn = new Date(
                  $scope.invoice.trip[i].unloadedOn
                );
              }
            }
            if ($scope.invoice.lrDate) {
              $scope.invoice.lrDate = new Date($scope.invoice.lrDate);
              $scope.invoice.consignorInvoiceDate = new Date(
                $scope.invoice.consignorInvoiceDate
              );
              $scope.invoice.gatePassDate = new Date(
                $scope.invoice.gatePassDate
              );
            }
    ...
    $scope.addFromAndTo = function () {
      if (
        !$scope.invoice.trip[$scope.invoice.trip.length - 1].from ||
    ...
    $scope.add_editInvoice = function () {
      if ($stateParams.id) {
        if ($scope.invoice.tripId) {
          ...
          for (var i = 0; i < $scope.invoice.trip.length; i++) {
            $scope.invoice.trip[i].amountPerTonne = parseFloat(
              $scope.invoice.trip[i].ratePerTonne *
              $scope.invoice.trip[i].tonnage
            );
          }
        } else {
          $scope.invoice.partyId = $scope.invoice.partyId._id;
          $scope.invoice.totalAmount =
            $scope.invoice.rate * $scope.invoice.quantity;
          for (var i = 0; i < $scope.invoice.trip.length; i++) {
            if ($scope.invoice.trip[i].vehicleNo.registrationNo) {
              $scope.invoice.trip[i].vehicleNo =
                $scope.invoice.trip[i].vehicleNo.registrationNo;
            }
          }
        }
...
app.controller('invoicesListController', [
  '$scope',
  '$rootScope',
  'InvoiceService',
  '$state',
  ...
  function (
    ...
    $scope.getInvoice = function (party) {
      ...
      InvoiceService.getInnvoiceByParty(
        params,
        function (success) {
          ...
          if (success.data.status) {
            ...
            $scope.PdfTotalAmount = success.data.totalAmount;
            console.log("totallllllllllllllllll", $scope.PdfTotalAmount);
    ...
    $scope.downloadDetails = function () {
      ...
      window.open(
        '/v1/invoices/downloadDetails?start=' +
        params.fromDate +
        '&end=' +
        params.toDate +
        '&partyId=' +
    ...
    $scope.printArea = function () {
      var w = window.open();
      w.document.write(
        document.getElementsByClassName('report_left_inner')[0].innerHTML
      );
      w.print();
      w.close();
    };
